...
const WHATSAPP_META_INICIAL = { phoneNumberId: "", graphVersion: "v24.0", templateName: "cotacao_fornecedor", pedidoTemplateName: "pedido_compra", templateLanguage: "pt_BR", accessToken: "", verifyToken: "", appSecret: "" };
WHATSAPP_META_INICIAL.logisticaTemplateName = "logistica_rastreio";
...
function WhatsappMetaConfiguracao({ apiStatus }) {
  ...
  function aplicarStatus(item) {
    setStatus(item);
    setFormulario(prev => ({ ...prev, phoneNumberId: item.phoneNumberId || "", graphVersion: item.graphVersion || "v24.0", templateName: item.templateName || "cotacao_fornecedor", pedidoTemplateName: item.pedidoTemplateName || "pedido_compra", templateLanguage: item.templateLanguage || "pt_BR", accessToken: "", verifyToken: "", appSecret: "" }));
    setFormulario(prev => ({ ...prev, logisticaTemplateName: item.logisticaTemplateName || "logistica_rastreio" }));
  }
  ...
  async function salvar() {
    ...
    if (!formulario.phoneNumberId.trim() || !formulario.graphVersion.trim() || !formulario.templateName.trim() || !formulario.pedidoTemplateName.trim() || !formulario.templateLanguage.trim()) { setFeedback({ tone: "red", text: "Preencha o Phone Number ID, vers\u00e3o, template e idioma." }); return; }
  ...
  async function remover() {
    ...
    setCarregando(true);
    try { await api.removerWhatsappMeta(); aplicarStatus({ configurada:false, phoneNumberId:"", graphVersion:"v24.0", templateName:"cotacao_fornecedor", pedidoTemplateName:"pedido_compra", templateLanguage:"pt_BR", webhookUrl:status?.webhookUrl }); setFeedback({ tone:"green", text:"Configura\u00e7\u00e3o do WhatsApp removida." }); }
  ...
  return <Card className="overflow-hidden border-emerald-200 dark:border-emerald-500/20">
    ...
    <div className="space-y-4 p-4 sm:p-5">
      ...
      <label className="block text-xs text-slate-500">Template de rastreio (nome, c&oacute;digo e link)<input value={formulario.logisticaTemplateName} onChange={e=>alterar("logisticaTemplateName",e.target.value)} placeholder="logistica_rastreio" className={campo}/></label>
