...
function Receitas({ receitas, produtos, categorias, fichas, estoqueItens, onCadastrarItem, onAtualizarItem, onExcluirProduto, onSalvarProduto, onSalvarCategoria, onSalvarFicha }) {
  ...
  const [feedback, setFeedback] = useState(null);
  const [funcionariosResponsaveis, setFuncionariosResponsaveis] = useState([]);
  const [funcionarioResponsavelId, setFuncionarioResponsavelId] = useState("");
  useEffect(() => { if (api.enabled) api.getFuncionariosAtivos().then(setFuncionariosResponsaveis).catch(() => setFuncionariosResponsaveis([])); }, []);
  const numero = valor => Number(String(valor || "").replace(",", ".")) || 0;
  ...
  function editarFicha(ficha) {
    ...
    setFuncionarioResponsavelId(ficha.funcionarioResponsavelId || "");
  ...
  function salvarFicha(e) {
    ...
    const resultado = onSalvarFicha({
      ...
      funcionarioResponsavelNome: funcionariosResponsaveis.find(funcionario => funcionario.id === funcionarioResponsavelId)?.nome || "",
      ...
      <div className="min-h-[720px] bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 flex font-sans">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            {active === "receitas" && <Receitas receitas={initialReceitas} produtos={produtos} categorias={categoriasProduto} fichas={fichas} estoqueItens={estoqueItens} onCadastrarItem={handleCadastrarInsumo} onAtualizarItem={handleAtualizarInsumo} onExcluirProduto={handleExcluirProduto} onSalvarProduto={handleSalvarProduto} onSalvarCategoria={handleSalvarCategoria} onSalvarFicha={handleSalvarFicha} />}
